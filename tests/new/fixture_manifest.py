...
mapping. The transport (:mod:`tests.new.fixture_transport`) is intentionally
...
from tests.new.fixture_transport import FixtureValue
...
# tests/new/fixture_manifest.py → tests/new → tests → project root
...
def _resolve_play_by_play() -> tuple[list[Case] | None, str | None]:
    ...
    for dir_ in team_dirs:
        ...
        index_url = f"{BASE_URL}/boxscores/?month={month}&day={day}&year={year}"
...
def _resolve_season_schedule() -> tuple[list[Case] | None, str | None]:
    ...
    for year_dir in year_dirs:
        ...
        month_hrefs = [
            h
            for h in all_hrefs
            if re.match(rf"^/leagues/NBA_{year}_games-[a-z]+\.html$", h)
        ]
        if not month_hrefs:
            continue
        ...
        for href in month_hrefs:
            ...
            month_match = re.match(rf"^/leagues/NBA_{year}_games-([a-z]+)\.html$", href)
            if not month_match:
