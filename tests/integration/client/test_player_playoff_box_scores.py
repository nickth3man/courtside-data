...
class RussellWestbrook2020IncludingInactiveGames(TestCase):
    ...
    def test_json_file_output(self, m):
        ...
        try:
            ...
            self.assertTrue(filecmp.cmp(output_file_path, self.expected_output_json_file_path))
        finally:
    ...
    def test_csv_file_output(self, m):
        ...
        try:
            ...
            self.assertTrue(filecmp.cmp(output_file_path, self.expected_output_csv_file_path))
        finally:
            os.remove(output_file_path)
...
class Giannis2020(TestCase):
    ...
    def test_inactive_games_are_removed_by_default(self, m):
        m.get("https://www.basketball-reference.com/players/a/antetgi01/gamelog/2020", text=self._html, status_code=200)
    ...
    def test_inactive_games_are_included_when_option_is_explicitly_selected(self, m):
        m.get("https://www.basketball-reference.com/players/a/antetgi01/gamelog/2020", text=self._html, status_code=200)
        results = playoff_player_box_scores(
            player_identifier="antetgi01", season_end_year=2020, include_inactive_games=True
        )
        self.assertIsNotNone(results)
