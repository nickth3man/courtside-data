...
from courtside_data.client import player_career_stats, search, standings
...
def test_csv(name, func, *args):
    ...
    func(*args, output_type=OutputType.CSV, output_file_path=tmp, output_write_option=OutputWriteOption.WRITE)
    with open(tmp) as f:
        content = f.read()
        print(f"\n=== {name} ===")
        print(content)
