"""``ENDPOINTS`` dict — the declarative registry of Basketball-Reference table endpoints.

Split out of the legacy monolithic :mod:`courtside_data.endpoints` module so
the per-endpoint ``output.columns``/``schemas`` imports are isolated from the
``TableEndpoint`` machinery in :mod:`courtside_data.endpoints._table`.
"""
