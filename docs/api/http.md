...
The Team Hub mirrors the Player Hub route shape with team identifiers
and team-scoped datasets.
...
`/api/teams/{team_identifier}/{dataset}` only accepts `scope="team"`
datasets (`contracts`, `franchise-history`). Season-scoped team
datasets must use the `/seasons/{season_end_year}/...` route.

## Codegen integration

`npm run gen:api` produces `ui/src/lib/openapi-types.ts` from this
server's `/openapi.json` endpoint:
...
CI runs the same generation flow in the `api-codegen` job and fails if
`ui/src/lib/openapi-types.ts` differs from the committed file. The
Pydantic models in `courtside_data/server/models.py` and
`courtside_data/server/team_models.py` remain the authoritative server
contract while the UI finishes migrating away from hand-written mirrors.
